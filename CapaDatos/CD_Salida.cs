    ...
    public class CD_Salida
    {
        public DataTable Listar()
        {
            try
            {
                ...
                query.AppendLine("SELECT * FROM Salida");
        ...
        public int Registrar(Salida obj, out string Mensaje)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("RegistrarSalida", con.conectar());
        ...
        public int ultimoRegistro()
        {
            try
            {
                Conexion con = new Conexion();
                StringBuilder query = new StringBuilder();
                query.AppendLine("SELECT MAX(Id) FROM Salida");
        ...
        public DataTable ListarPorFecha(string fechaInicio, string fechaFin)
        {
            try
            {
                Conexion con = new Conexion();
                StringBuilder query = new StringBuilder();
                query.AppendLine("SELECT * FROM Salida WHERE Fecha BETWEEN @FechaInicio AND @FechaFin");
                SqlCommand cmd = new SqlCommand(query.ToString(), con.conectar());
        ...
        public DataTable FiltrarSalidas(string fechaInicio, string fechaFin, string orderBy, string order)
        {
            try
            {
                Conexion con = new Conexion();
                StringBuilder query = new StringBuilder();
                query.AppendLine("SELECT Id as 'N° Salida', Total, Fecha FROM Salida WHERE Fecha BETWEEN @FechaInicio AND @FechaFin");
